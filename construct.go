...
//	}
//	fieldcraft.Str(dec, "Domain", get, set, fieldcraft.Meta{Flag: "domain"})
//
...
//
//	decb := fieldcraft.Decided[*Config, bool]{
//	    Read:  func(s *Config, n string) *bool { /* from decisions map */ },
//	    Write: func(s *Config, n string, v bool) { /* to decisions map */ },
//	}
//	fieldcraft.Bool(decb, "OAuth", getOAuth, setOAuth, fieldcraft.Meta{Flag: "oauth"})
//
...
func applyDefault[S any, T any](spec *fieldSpec[S, T], d any) {
	def, ok := d.(T)
	if !ok {
		panic(fmt.Sprintf("fieldcraft: Meta.Default for %q is %T, want %T", spec.Name, d, *new(T)))
	}
	spec.DefaultVal = &def
}
