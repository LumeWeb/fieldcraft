...
// This file defines a declarative field-resolution primitive for a workflow
// step. A step describes the values it needs and the framework resolves each
// one in precedence order — provider-derived (precedence 0), host switch (and
// its process-env Sources), interactive prompt, persisted env file — instead of
// the step hand-rolling a per-field conditional on the state and
// interactivity.
//
...
//	               loaded provider state, resolved credential, a default) or
...
//
// The split exists because configurers write derived values (a resolved URL, a
// provisioned-state default, resolved credentials) that would be mistaken for
// operator decisions by a naive "non-nil = decided" tri-state.
//
// S MUST be a pointer type; a run that copies state S by value still aliases
// the same struct when S is a pointer, so Commit persists. Accessors take S
// (the pointer) directly.

// ValueSource abstracts where an operator-supplied value comes from. The core
// package does not import a CLI framework; the host adapts its command and env
// layers to this interface. Sources are strings (flags and env are strings);
// Field.Parse converts them to T.
type ValueSource interface {
	...
	// EnvFile returns a persisted env-file value (a file the host writes and
	// later reads at runtime). ok=false when the key is absent.
...
// Field describes one configurable value a workflow step needs. See the
// provenance model above for Decided vs Operational. S MUST be a pointer type.
type Field[S any, T any] struct {
	...
	// object's Required array (which a host keys its elicitation on).
	...
	// derivation (loaded provider state, a URL resolved from an account API, a
	// host-assigned default, a resolved credential). It runs at precedence 0 —
	// before the switch — and only
...
func resolveField[S any, T any](src ValueSource, s S, f *Field[S, T], headless bool) (fieldOutcome[T], error) {
	...
	// Resolves a derived value (provider state, resolved URL, a default) BEFORE
	// the switch, and only when the field carries no operator
...
//
// S MUST be a pointer type; the run passes S by value but with
