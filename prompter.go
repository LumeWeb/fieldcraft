...
// Prompter is the single window between framework logic and the terminal. A
// gathering run owns exactly one Prompter (its one rendering channel); any step
// — top-level or spliced in as a "sub-workflow" — requests user input through it
// instead of spawning its own terminal widget. This is what lets a host embed
// steps from another workflow without two independent rendering systems
// fighting over one terminal.
...
// WithPrompter attaches a Prompter to ctx. The runner sets this on the context
// handed to every step; nested/spliced steps inherit it, so an embedded
// sub-workflow shares the host's single terminal channel.
