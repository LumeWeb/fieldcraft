...
// driver (AnyField) so a workflow step can define every field once and resolve
// a heterogeneous set (string, bool, []T, enum...) in a single pass.
//
...
// registry of these once and derives every Field view (and the
...
type AnyField[S any] interface {
	...
	// lists it in the object's Required array, which keys host elicitation).
	Required() bool
...
func (e *erasedField[S, T]) resolve(ctx context.Context, src ValueSource, s S, headless bool) (resolvedField, error) {
	...
	if !headless && e.f.Prompt != nil {
		p := PrompterFrom(ctx)
		if p == nil {
			return rf, fmt.Errorf("fieldcraft.Gather: field %q needs a prompt but no Prompter is bound to ctx", e.f.Name)
		}
		...
		if e.f.OptionsFunc != nil && len(options) == 0 {
			return rf, fmt.Errorf("fieldcraft.Gather: select field %q has no options to choose from", e.f.Name)
		}

		if e.f.Prompt.Multi {
			// Multi-select: checked option labels -> ParseMulti -> T.
			if e.f.ParseMulti == nil {
				return rf, fmt.Errorf("fieldcraft.Gather: multi-select field %q needs Field.ParseMulti", e.f.Name)
			}
			if len(options) == 0 {
				return rf, fmt.Errorf("fieldcraft.Gather: multi-select field %q has no options to choose from", e.f.Name)
			}
			var pre []string
			...
			if !okP || !e.f.settle(&oc, s, val, srcPrompt, headless) {
				return rf, fmt.Errorf("fieldcraft.Gather: invalid selection entered for field %q", e.f.Name)
			}
		...
		if !okP || !e.f.settle(&oc, s, chosen, srcPrompt, headless) {
			return rf, fmt.Errorf("fieldcraft.Gather: invalid value entered for field %q", e.f.Name)
		}
		rf.decided = oc.decided
		rf.operative = oc.operative
		rf.usedSource = oc.usedSource
